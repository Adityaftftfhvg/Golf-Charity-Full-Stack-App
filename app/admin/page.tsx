...
// NO direct email imports — emails are called via API routes to keep Resend server-side only
...
export default function AdminPanel() {
  ...
  const fetchWinners = async () => {
    const { data } = await supabase
      .from("winners").select("*, users(email)").order("created_at", { ascending: false });
    setWinners(data || []);
  };
  ...
  // --- DRAW FUNCTIONS ---
  const simulateDraw = () => setSimResult(generateDraw());
  ...
  const publishDraw = async () => {
    ...
    // Fetch active users WITH email for matching and emailing
    const { data: activeUsers } = await supabase
      .from("users").select("id, email").eq("subscription_status", "active");
    ...
    for (const user of activeUsers) {
      const { data: scores } = await supabase
        .from("scores").select("score").eq("user_id", user.id)
    ...
    const insertWinners = async (userIds: string[], matchType: number, totalPrize: number) => {
      ...
      for (const uid of userIds) {
        await supabase.from("winners").insert({
          draw_id: draw.id, user_id: uid, match_type: matchType,
          prize_amount: share, status: "pending",
        });
      }
    };
    ...
    // ✅ Send draw result emails via API route (server-side only — no client Resend crash)
    try {
      const emailPayload = activeUsers.map((user) => {
        let matched = 0;
        let prize = 0;
        if (match5Winners.includes(user.id)) {
          matched = 5; prize = match5Winners.length > 0 ? pool5 / match5Winners.length : 0;
        } else if (match4Winners.includes(user.id)) {
          matched = 4; prize = match4Winners.length > 0 ? pool4 / match4Winners.length : 0;
        } else if (match3Winners.includes(user.id)) {
          matched = 3; prize = match3Winners.length > 0 ? pool3 / match3Winners.length : 0;
        }
        return {
          email: user.email,
          month: now.toLocaleString("default", { month: "long" }),
      ...
      console.error("Draw emails failed:", e);
    }
  ...
  // ✅ Payout email via API route (server-side only)
        ...
        {tabs.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`px-4 py-2 rounded-t-lg text-sm font-medium transition ${
              tab === t.key ? "bg-slate-800 text-white" : "text-gray-400 hover:text-white"
            }`}>
            {t.label}
          </button>
        ))}
                ...
                <tbody>
                  {users.map((u) => (
                    <tr key={u.id} className="border-b border-slate-700/50 hover:bg-slate-700/30">
                      ...
                      <td className="py-3 pr-4 text-gray-400">{u.charity_percentage || 10}%</td>
                      <td className="py-3 pr-4">
                        <span className={`px-2 py-0.5 rounded-full text-xs ${
                          u.role === "admin"
                            ? "bg-purple-500/20 text-purple-400" : "bg-slate-600 text-gray-400"
                        }`}>{u.role || "user"}</span>
                      </td>
                      <td className="py-3">
        ...
        {tab === "draws" && (
          <div>
            ...
            <div className="bg-slate-700 p-5 rounded-xl mb-6">
              <h3 className="text-sm text-gray-400 uppercase tracking-wide mb-4">Run a Draw</h3>
              {simResult && (
                <div className="mb-4">
                  <p className="text-sm text-gray-400 mb-2">Simulated numbers:</p>
                  <div className="flex gap-2">
                    {simResult.map((n, i) => (
                      <div key={i} className="w-10 h-10 bg-purple-600 rounded-full flex items-center justify-center font-bold">
                        {n}
                      </div>
                    ))}
                  </div>
              ...
              <div className="flex gap-3">
                <button onClick={simulateDraw} disabled={publishing}
              ...
              {publishing && (
                <p className="text-xs text-gray-400 mt-3">Calculating prize pools, matching winners, sending emails...</p>
              )}
            </div>
                ...
                <div key={d.id} className="bg-slate-700 p-4 rounded-xl flex justify-between items-center">
                  <div>
                    ...
                    <div className="flex gap-1 mt-1">
                      {(d.numbers || []).map((n, i) => (
                        <span key={i} className="text-xs bg-purple-500/30 text-purple-300 px-2 py-0.5 rounded">{n}</span>
                      ))}
                    </div>
                  </div>
            ...
            <div className="bg-slate-700 p-5 rounded-xl mb-6">
              ...
              <div className="space-y-3">
                <input placeholder="Charity name" value={charityName}
                  onChange={(e) => setCharityName(e.target.value)} className="w-full p-3 rounded-lg text-black" />
                ...
                <div key={w.id} className="bg-slate-700 p-4 rounded-xl">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      ...
                      <p className="text-gray-400 text-sm">
                        {w.match_type}-number match · ₹{(w.prize_amount || 0).toFixed(2)}
                      </p>
                    </div>
                    <span className={`px-2 py-0.5 rounded text-xs ${
                  ...
                  {w.proof_url && (
                    <a href={w.proof_url} target="_blank"
                      className="text-xs text-purple-400 hover:underline block mb-3">View proof →</a>
                  )}
          ...
          <div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="bg-slate-700 p-5 rounded-xl">
                <p className="text-gray-400 text-sm mb-3">Subscription breakdown</p>
                <div className="space-y-2">
                  {["active", "inactive"].map((s) => {
                    const count = users.filter((u) => u.subscription_status === s).length;
                    return (
